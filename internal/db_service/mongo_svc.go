...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("PREMISES_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("PREMISES_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("PREMISES_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("PREMISES_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("PREMISES_API_MONGODB_DATABASE", "vecnehladny-premises-wl")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("PREMISES_API_MONGODB_COLLECTION", "premises")
	}

	if svc.Timeout == 0 {
		seconds := enviro("PREMISES_API_MONGODB_TIMEOUT_SECONDS", "10")
